...
it happens?
Ans: If a ball is fast enough, it is possible that the ball in one time frame is completely inside the arena, but in the next time frame
its position changed so much that it ended up completely outside. 
What we do is update velocity --> update position --> check for collision
so if the ball goes out directly, it would have updated its position twice on the normal velocity before the velocity is reversed and then used for turning back, so the ball would start kind of like twitching outside the arena constantly changing its velocity back and forth as its outside the arena and the according to the logic its sensing a collision with the wall every timeframe so its reversing its velocity along the normal.
...
from?
Ans. In my observation of my project, the balls eventually lose energy. This is because the way we are doing the "integration" is not perfectly accurate. There is some amount of error as dt cannot be infinetly small and obviously the velocity is not constant over this relatiely big dt interval. Integration using the explicit euler method adds energy and with the symplectic euler method diminishes energy as far as I have seen. I cannot explain why though. Also when the particle kind of phases a little through the arena when we check for collision, we immediately try to reposition it at the boundary of the arena to prevent the ball from sticking. So we changed its place without changing its velocity in an instant even when acceleration is there. This also contributes to the error in the energy conversation.
